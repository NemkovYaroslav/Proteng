...
void ParticleSystem::CreateBuffers()
{
	...
	// т.к. дальше будем мен€ть местами буфферы, то делаем перенастройку
	srvSrc = srvFirst;
	uavSrc = uavFirst;
	srvDst = srvSecond;
	uavDst = uavSecond;
	...
	// здесь &MaxParticlesCount - значение счЄтчика (если хотим сохранить значение, которые было до этого, то ставим здесь -1)
	...
	// буфер, с помощью которого узнаем значени€, хранищиес€ в счЄтчике uavSrc
	...
	// UNORDERED ACCESS VIEW дл€ INJECTION
...
void ParticleSystem::Update(float deltaTime)
{
	...
	Game::GetInstance()->GetRenderSystem()->context->CSSetUnorderedAccessViews(0, 1, &uavSrc, &counterKeepValue); // читаем частицы, сохран€ем счетчик через -1 // particlesBufSrc
	...
	// если ещЄ есть необработанные частицы
	if (groupSizeX > 0)
	{
